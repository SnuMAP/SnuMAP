...
#                      help='Pick task colors (RGB) from a Gnuplot palette specified in FILE (see Gnuplot documentation')
    ...
    parser.add_option('-i', '--int',
                      action='store_true', dest='intsort',
                      default=False,
                      help='Show resources in integer order')
    ...
    if options.intsort:
        resources = sorted(resources, cmp=_compare_keys)
    ...
    se_palette = {
                 ...
                 "se_red": (1.0, 0.0, 0.0),
                 "se_pink": (1.0, 0.0, 1.0),
                 "se_violet": (0.0, 0.0, 1.0),
                 "se_yellow": (1.0, 1.0, 0.0)
                 }
    se_gradient = ["se_green", "se_blue", "se_red",
                   "se_pink", "se_violet", "se_yellow"]
    se_palettedef = '( ' + \
                    ', '.join(('%d ' % n +
    ...
    # Pick colors from a pre-defined palette
        ...
        colors = dict([(t, '%0.2f' % (float(n)/len(tasks))) for n, t in enumerate(tasks)])
    ...
    keyPosition = 'outside top center'
    gridTics = 'xtics'
    ...
    plot_dimensions = [
                       ...
                       'set xlabel "%s" font "Times-Roman, 12"' % xlabel,
                       'set ylabel "%s" font "Times-Roman, 12"' % ylabel,
                       'set title "%s" font "Times-Roman, 14"' % title,
                       'set ytics %s' % ytics,
                       ...
                       'set key samplen 1 spacing 1 font "Times-Roman, 8"',
                       'set key below',
